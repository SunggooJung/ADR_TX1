...
#define _FORWARD            (float)  (       3.0)
#define _TURN               (float)  (       7.5)
...
int main(int argc, char** argv)
{
    ...
    while( ok() )
    {
	    ...
	    cout << "roll_err_m: " << pos_error_x_m << " roll_cmd: " << cmd.X_out << " pitch_cmd: " << cmd.Y_out << "\n";
            cout << "count: " << count_adhoc  <<" psi_cmd: " << cmd_pos_psi << "\n";
            count_mission_start = count_mission_start + 1;
            pos_error_x_m = sat(pos_error_x_m, 0.8);

            cmd_pos_psi = WP_psi[gate_num%2];
            cmd_pos_z   = WP_z[gate_num%2]  ;
...
/// ---------------------------------------Straight Section----------------------------------------------------------
            ...
            {	
                ...
                else if(count_adhoc > (_FORWARD)*SECOND && count_adhoc < (_FORWARD+_TURN)*SECOND)
                {
                    cout << "!!!!!!!!U-Turn!!!!!!!" << "\n";
                    cmd.X_out = 0.0;
                    cmd.Y_out = 0.0;
...
void updatedata(void)
{
    ...
    tx_data.CmdR_dps = sat(cmd.PSI_out, 60);
